...
{
	public int ID;
	public bool Ownership=false;	//whether this block is owned by this node
	public int collided = 0;		//The collided number 
		...
		if (MainController.FSM.IsInState (PuzzleState.GAME_STEP)) {
			...
			if(LocalPlayer.activePiece == ID){
				...
				if(Input.GetKey("left")){
					LocalPlayer.curOrientation.y += rotateRate*Time.deltaTime;
				}else if(Input.GetKey("right")){
					LocalPlayer.curOrientation.y -= rotateRate*Time.deltaTime;
				}
			}

			if(LocalPlayer.activePiece == ID || AgentPlayer.activePiece == ID){
				vibrating();
			}else{
				if(!GameInfo.blockSucceed[ID]){
					resetRotate();
