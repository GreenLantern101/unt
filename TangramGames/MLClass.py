...
class MLClass:
    ...
    def createMLmodel(self, fileName):
        ...
        wordFile = open('F:/agent/AI/Dialogue/TangramGames/wordArray.txt', 'w')
        for item in self.wordArray:
            wordFile.write("%s\n" % item)
        wordFile.close()
        biFile = open('F:/agent/AI/Dialogue/TangramGames/bigramArray.txt', 'w')
        for item in self.bigramArray:
            biFile.write("%s\n" % item)
        biFile.close()
        posFile = open('F:/agent/AI/Dialogue/TangramGames/posArray.txt', 'w')
        for item in self.posArray:
            posFile.write("%s\n" % item)
        posFile.close() 
        '''
        depFile = open('F:/agent/AI/Dialogue/TangramGames/depArray.txt', 'w')
        ...
        joblib.dump(pca, 'F:/agent/AI/Dialogue/TangramGames/pcaModel.pkl')
        
        #classification methods
        clf = SVC(gamma=2, C=1)
        clf.fit(self.featureList, self.actionArray)
        joblib.dump(clf, 'F:/agent/AI/Dialogue/TangramGames/clasModel.pkl') 

        
    def loadclassModel(self):
        clf = joblib.load('F:/agent/AI/Dialogue/TangramGames/clasModel.pkl') 
        return clf
    
    def loadpcaModel(self):
        pca = joblib.load('F:/agent/AI/Dialogue/TangramGames/pcaModel.pkl') 
        return pca
           
    def loadOtherInfor(self):
        self.wordArray = [line.rstrip('\n') for line in open('F:/agent/AI/Dialogue/TangramGames/wordArray.txt')]
        self.bigramArray = [line.rstrip('\n') for line in open('F:/agent/AI/Dialogue/TangramGames/bigramArray.txt')]
        self.posArray = [line.rstrip('\n') for line in open('F:/agent/AI/Dialogue/TangramGames/posArray.txt')]
        #self.depArray = [line.rstrip('\n') for line in open('F:/agent/AI/Dialogue/TangramGames/depArray.txt')]
