...
class setup:
    ...
    TrainfileName = 'F:/agent/AI/Dialogue/TangramGames/Collaborative_Game1.txt'
    logFileName = 'F:/agent/AI/Dialogue/TangramGames/logFile.txt'
    #train the model
    ...
    def Update(self):
        ...
        while LoopFlag:
            ...
            if len(speechData)>0:
                ...
                fp.write('time; ' + str(datetime.datetime.now().time()) + ', received data; ' + speechDataAscii + '\n')
                featureV = self.MLInstance.convert2Feature(speechDataAscii)
                featureVector = self.MLInstance.processFeature(self.pcaModel, featureV)
                actCategory = self.MLInstance.MLPredict(self.classModel,featureVector)
                actList = actCategory.tolist() #change the type from numpy.ndarray to list
                act = actList[0]
                sDataInstance = self.MLInstance.getResponse(act, speechDataAscii)
                print sDataInstance
                #responseData = sDataInstance.encode("utf-16") #need to convert to utf16 before send to C#
                fp.write('time; ' + str(datetime.datetime.now().time()) + ', send data; ' + sDataInstance+ '\n')
